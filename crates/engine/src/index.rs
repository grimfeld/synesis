...
pub struct Candidate {
    ...
    /// unused and used: placed, but not yet committed to the talk (PLAN §16.6).
    pub on_board: bool,
}
...
-- `kind` separates prose links from Board refs (PLAN §16.6): a document
...
impl Index {
    ...
    pub fn upsert(
        ...
        for victim in [Some(id.to_string()), old_id].into_iter().flatten() {
            ...
            // not this re-index's to clear (PLAN §16.6).
    ...
    pub fn candidates(&self, id: &str, lang: Lang, limit: usize) -> Result<Vec<Candidate>> {
        ...
        // material on the Board is placed, not used (PLAN §16.6).
    ...
    /// node itself stays on the Board, rendered as missing (PLAN §16.13).
...
mod tests {
    ...
    fn board_refs_are_not_prose_links() {
        ...
        // a Candidate: the Board is where nothing is committed (PLAN §16.6).
