...
fn dispatch(app: &AppHandle, cmd: &str, a: Value) -> Result<Value, String> {
    ...
    match cmd {
        ...
        "unlinked_mentions" => ok(unlinked_mentions(state, arg(&a, "id")?)?),
        "linkables" => ok(linkables(state, arg(&a, "id")?, arg(&a, "text")?)?),
        "ambiguous_titles" => ok(ambiguous_titles(state)?),
        ...
        "timeline_tags" => ok(timeline_tags(state)?),
        ...
        "boards_referencing" => ok(boards_referencing(state, arg(&a, "id")?)?),
        "board_excerpts" => ok(board_excerpts(state, arg(&a, "refs")?)?),
