# OneDrive, Google Drive ou Dropbox sur un Mac
...
Sur macOS ces apps vivent sous `~/Library/CloudStorage/` (OneDrive-Personal, GoogleDrive-<compte>, Dropbox) ; Synesis y regarde.

1. Installez le client de votre fournisseur, réglez-le pour garder les fichiers sur ce Mac, et créez le coffre dans son dossier :
   - **OneDrive**
     - Installez depuis le Mac App Store, connectez-vous.
     - Clic droit sur le dossier OneDrive dans le Finder → **Toujours conserver sur cet appareil**.
     - Créez le coffre dans `~/Library/CloudStorage/OneDrive-Personal/Synesis`.
   - **Google Drive pour ordinateur**
     ...
     - **Préférences → Google Drive → Dupliquer les fichiers**.
     - Créez le coffre dans `~/Library/CloudStorage/GoogleDrive-<vous>/Mon Drive/Synesis`.
   - **Dropbox**
     - Installez depuis [dropbox.com/install](https://www.dropbox.com/install), connectez-vous.
     - **Préférences → Synchronisation → Nouveaux fichiers : Local**.
     - Créez le coffre dans `~/Library/CloudStorage/Dropbox/Synesis` (ou `~/Dropbox/Synesis` sur d'anciennes installations).
