...
4. **Repo layout**: `skins/<slug>.json`, one Skin per file in the Export
   format; `skins/index.json` generated by `npm run skins:index` with each
   Skin's `id`, `name`, `author`, file, and both sides' three seeds (for
   swatches). `skins/README.md` explains submitting and credits palettes.
...
6. **UI**: a *Gallery* button beside Import in Settings -> Appearance and a
   "Browse Skin gallery" Command. It opens a **non-modal sheet** without an
   overlay (right on desktop, bottom on phone) so the app stays visible.
   Rows: name, author, light and dark swatches, and *Install* or
   *Installed · Wear*. Loading shows skeleton rows; a failed fetch shows
   "Couldn't reach the gallery" and Retry.
...
8. **Nord** (`author: "grimfeld"`, palette by Arctic Ice Studio, MIT, credited
   ...
   `typeChroma: 0.7`. Typography unchanged.
9. **Paper** (`author: "grimfeld"`): a book page, not Sepia's old page.
   ...
   sides: Source Serif 4 prose, sans UI, line height 1.7, line width ~34em,
   `typeChroma: 0.75`. Skins cannot carry texture; the paper feel is colour
   and type.
...
11. **Tests**: `skins-gallery.cy.ts` and the Tutorial walk stub the gallery
    commands with a fixed index and never reach GitHub; a `locales.cy.ts`
    case for the sheet at phone width in French.
