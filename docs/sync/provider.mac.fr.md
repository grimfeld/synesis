...
1. Installez depuis le Mac App Store, connectez-vous.
2. Clic droit sur le dossier OneDrive dans le Finder → **Toujours conserver sur cet appareil**.
3. Créez le coffre dans `~/Library/CloudStorage/OneDrive-Personal/Synesis`.
...
2. **Préférences → Google Drive → Dupliquer les fichiers**.
3. Créez le coffre dans `~/Library/CloudStorage/GoogleDrive-<vous>/Mon Drive/Synesis`.

## Dropbox

1. Installez depuis [dropbox.com/install](https://www.dropbox.com/install), connectez-vous.
2. **Préférences → Synchronisation → Nouveaux fichiers : Local**.
3. Créez le coffre dans `~/Library/CloudStorage/Dropbox/Synesis` (ou `~/Dropbox/Synesis` sur d'anciennes installations).
