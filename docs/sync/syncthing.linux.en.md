...
1. Debian / Ubuntu: `sudo apt install syncthing`. Fedora: `sudo dnf install syncthing`. Arch: `sudo pacman -S syncthing`. Or use the official repository at [apt.syncthing.net](https://apt.syncthing.net/) for current versions.
2. Start it for your user and at login: `systemctl --user enable --now syncthing.service`.
3. Open the web UI at [http://127.0.0.1:8384](http://127.0.0.1:8384).
...
Suggested vault path on Linux: `~/Sync/Synesis`.
